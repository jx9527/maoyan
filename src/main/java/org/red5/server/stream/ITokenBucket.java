 ...
 * 基本上，令牌桶用于控制流、连接或客户机使用的带宽。有一个后台线程，
 * 根据bucket的配置将令牌分发到系统中的bucket。配置包括令牌的分发速度。
 * 例如，当流需要发送数据包时，将计算数据包的字节计数，并且每个字节对应于
 * 桶中的令牌。流被分配一个桶，桶中的令牌在发送包之前被获取。
 * 因此，如果配置中的速度（或带宽）很低，流就不能快速发送数据包。
